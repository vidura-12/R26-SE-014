...
def process_farm(farm) -> int:
    ...
    try:
        ...
        print(f"  {len(cells)} grid cells  (~ {cell_m} m x {cell_m} m each)\n")
...
def main():
    ...
    grand_total = 0
    max_workers = 6  # start small (2–4), then increase

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(process_farm, farm) for farm in farms]
